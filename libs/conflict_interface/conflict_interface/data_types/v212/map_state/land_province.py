...
def requires_ownership(func):
    ...
    @wraps(func)
    def wrapper(self: "LandProvince", *args, **kwargs):
        if self.is_owner():
...
class LandProvince(GameObject):
    ...
    def cancel_construction(self) -> tuple[Optional[int], UpdateProvinceActionResult]:
        """
            ...
            Parameters:
                self (LandProvince): Represents the current province instance where the
    ...
    def update(self, other: "LandProvince", path: list[PathNode] = None, rp: BidirectionalReplayPatch = None):
        for updateable_key in LandProvince.updateable_keys:
