...
void ObservationSession::process_successful_response(GameServerResult& result) {
    ...
    // Get current timestamp in milliseconds (used for Redis)
...
asio::awaitable<ObservationResult> ObservationSession::run_update_async() {
    ...
    try {
        ...
        // If we detected a map id, ensure static map data is available (cached/uploaded)
        if (!result.map_id.empty()) {
            ensure_static_map_data(*api_, result.map_id);
        }
