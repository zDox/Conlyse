...
public:
    ...
    StaticMapCache(const std::string& cache_dir, const S3Config& s3_config, std::shared_ptr<DbClient> db_client);
    ~StaticMapCache();
    
    bool is_cached(const std::string& map_id) const;
    std::string save(const std::string& map_id, const json& static_map_data);
    
private:
    bool upload_to_s3(const std::string& local_path, const std::string& map_id);
    void ensure_bucket_exists();
    
    std::string cache_dir_;
    std::set<std::string> saved_ids_;
